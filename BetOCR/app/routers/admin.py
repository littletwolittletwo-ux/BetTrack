...
def list_sets_route(active: bool | None=None, db: Session = Depends(get_db), user = Depends(current_user)):
...
def cleanup_images_endpoint(retention_days: int = 7, admin = Depends(require_admin)):
...
def cleanup_orphaned_endpoint(admin = Depends(require_admin)):
