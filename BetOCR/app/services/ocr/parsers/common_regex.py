...
STAKE_RE = r"(stake|wager|bet|amount|win\s*•\s*stake)\s*[:\-•]?\s*\$?\s*([0-9]+(?:\.[0-9]{1,2})?)"
ODDS_RE  = r"(odds|price|@|multi\s*@|game.*@)\s*[:\-•]?\s*([0-9]+(?:\.[0-9]{1,3})?)"
RET_RE   = r"(potential\s*return|payout|return|bet\s*return|winnings)\s*[:\-•]?\s*\$?\s*([0-9]+(?:\.[0-9]{1,2})?)"
RESULT_RE = r"(result|status|outcome|state)\s*[:\-•]?\s*(won|lost|void|cash.?out|pending|resulted|winning)"
CASHOUT_RE = r"(cash.?out|early\s*payout)\s*[:\-•]?\s*\$?\s*([0-9]+(?:\.[0-9]{1,2})?)"
COMM_RE = r"(commission|comm|fee)\s*[:\-•]?\s*([0-9]+(?:\.[0-9]+)?)\s*%?"
...
BONUS_RE = r"(bonus|promo|boost)\s*[:\-•]?\s*\$?\s*([0-9]+(?:\.[0-9]{1,2})?)"
...
def parse_common(text: str) -> Dict[str, Any]:
 ...
 if not result["result_status"]:
     ...
     if re.search(r"(✓|tick|win|won|winning)", t, re.I):
         result["result_status"] = "won" 
     elif re.search(r"(✗|x|cross|lose|lost|losing)", t, re.I):
         result["result_status"] = "lost"
...
def parse_enhanced(text: str) -> Dict[str, Any]:
    ...
    # THIRD PRIORITY: Win amount detection for formats like "Win +$115.00"  
        ...
        elif not result.get("result_status") and ("✓" in text or "tick" in t):
            result["result_status"] = "won"
